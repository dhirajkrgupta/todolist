...
export default function TodoCard(props) {
  ...
  return (
    <div className="todo-card rounded-xl my-2 mx-2 p-2 flex border-2 border-white items-center">
      ...
      <button className="w-fit flex-none  text-xl rounded-2xl" onClick={()=>{editTask(index)}}><FaEdit /></button>
      <button className="w-fit flex-none  text-xl rounded-2xl" onClick={() => removeTask(index)}><MdDelete /></button>
    </div>
  );
}
